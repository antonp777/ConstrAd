...
from core.schemas.ScOrder import OrderRead, OrderCreate
...
async def get_order_by_id(session: Annotated[AsyncSession, Depends(db_helper.session_getter)], order_id: int):
...
async def get_order_by_user_id(session: Annotated[AsyncSession, Depends(db_helper.session_getter)], user_id: int):
...
async def create_order(session: Annotated[AsyncSession, Depends(db_helper.session_getter)], order_create: OrderCreate):
